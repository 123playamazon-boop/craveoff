...
const PUSHCUT_WEBHOOK_URL = process.env.PUSHCUT_WEBHOOK_URL; // Pushcut notification URL
...
export default async function handler(req, res) {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    // Pushcut notification (iPhone push on sale)
    if (PUSHCUT_WEBHOOK_URL) {
      const email = session.customer_email || (session.customer_details && session.customer_details.email) || '(no email)';
      try {
        const pr = await fetch(PUSHCUT_WEBHOOK_URL, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            title: `🍇 Venda Crave-Off!`,
            text: `$${amount} · ${email}`,
          }),
        });
        console.log('[stripe-webhook] Pushcut →', pr.status);
      } catch (err) {
        console.error('[stripe-webhook] Pushcut error:', err);
      }
    }
  }
